...
* Authorization 헤더에는: Access Token(JWT) 응답합니다. 로그인 이후 요청에 Authorization 헤더에 access-token 으로 로그인을 식별
...
Kakao 로그인:: https://www.bad-request.kr/api/v2/oauth/authorization/kakao?redirect_uri=http://localhost:3000/redirect
Github 로그인:: https://www.bad-request.kr/api/v2/oauth/authorization/github?redirect_uri=http:http://localhost:3000/redirect
